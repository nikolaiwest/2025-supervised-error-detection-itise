"""
Binary classification: 50 normal vs 50 faulty samples for each class value.
...
def run_binary_vs_50(model_selection="paper", save_results=True):
    ...
    if save_results:
        ensure_directory("results/binary/50vs50")
        ensure_directory("results/binary/50vs50/images")
    ...
    for class_value in sorted(set(class_values)):
        ...
        for model_name, model in model_dict.items():
            ...
            try:
                ...
                if save_results:
                    save_path = f"results/binary/50vs50/images/confusion_matrix_{class_value}_{model_name}.png"
    ...
    if save_results and not results_df.empty:
        save_results_with_plots(results_df, "results/binary", "50vs50")
...
if __name__ == "__main__":
    # Run the experiment
    results = run_binary_vs_50(model_selection="paper", save_results=True)
