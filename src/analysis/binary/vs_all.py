...
def run_binary_vs_all(model_selection="paper", save_results=True):
    """
    Run binary classification: Each class's faulty samples vs ALL normal samples from ALL classes.

    This differs from run_binary_vs_ref in that it uses all normal samples across all
    classes as the reference set, rather than just the normal samples from the current class.
    ...
    logger.info(
        ...
        f"Each class's faulty vs ALL normal samples (model selection: {model_selection})"
    )
    ...
    all_normal_mask = scenario_condition == "normal"
    all_normal_indices = np.where(all_normal_mask)[0]
    all_normal_torque = torque_values[all_normal_indices]

    logger.info(f"Found {len(all_normal_indices)} normal samples across all classes")
    ...
    for class_value in sorted(set(class_values)):
        ...
        # Filter data to get only FAULTY samples for this class
        ...
        # Get only faulty samples from this class
        faulty_mask = filtered_label_values != "normal"
        class_faulty_indices = np.where(faulty_mask)[0]

        # Skip if no faulty samples for this class
        if len(class_faulty_indices) < 1:
            logger.warning(f"Skipping {class_value}: no faulty samples present.")
            continue

        class_faulty_torque = filtered_torque_values[class_faulty_indices]

        logger.info(
            f"- Using {len(all_normal_indices)} normal (ALL classes) and "
            f"{len(class_faulty_indices)} faulty samples (class {class_value})"
        )

        # Calculate class weights due to potential imbalance
        class_ratio = len(all_normal_indices) / len(class_faulty_indices)
        logger.info(f"- Class imbalance ratio (normal:faulty): {class_ratio:.2f}:1.00")

        # Combine all normal samples with this class's faulty samples
        x_values = np.vstack([all_normal_torque, class_faulty_torque])

        # Create binary labels: 0 for normal, 1 for faulty
        y_values = np.zeros(len(x_values), dtype=int)
        y_values[len(all_normal_indices) :] = 1  # Set labels for faulty samples to 1
        ...
        for model_name, model in model_dict.items():
            ...
            try:
                ...
                result_entry = {
                    ...
                    "normal_samples": len(all_normal_indices),
                    "faulty_samples": len(class_faulty_indices),
